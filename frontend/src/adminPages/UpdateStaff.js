...
const UpdateStaff = ({staffId ,onClose, onStaffUpdated }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  useEffect(()=>{
    const fetchStaff = async () => {
        try {
            const res = await axiosSecure.get(`/single-staff/${staffId}`);
            const staffData = res.data.data;
            console.log('see staff data', staffData);
            setStaffs(staffData);
            setFetching(false);
  ...
  const onSubmit = async (data) => {
    ...
    try{
      setLoading(true);
      await axiosSecure.put(`/update-staff/${staffId}`,formData,{
        headers:{
